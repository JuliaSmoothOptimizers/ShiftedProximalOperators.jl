...
mutable struct ShiftedNormL0{R <: Real, V0 <: AbstractVector{R}, V1 <: AbstractVector{R}, V2 <: AbstractVector{R}} <: ShiftedProximableFunction
  h::NormL0{R}
  x0::V0  # base shift (nonzero when shifting an already shifted function)
  x::V1   # current shift
  s::V2   # internal storage
  function ShiftedNormL0(h::NormL0{R}, x0::AbstractVector{R}, x::AbstractVector{R})  where {R <: Real}
    s = similar(x)
