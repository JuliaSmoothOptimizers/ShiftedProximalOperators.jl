...
shifted(
	...
	Δ::R,
	χ::Conjugate{IndBallL1{R}}
 ) where {R <: Real, RR <: AbstractVector{R}, I} = ShiftedGroupNormL2Binf(h, xk, zero(xk), Δ, χ,  false)
 shifted(
	h::NormL2{R},
	xk::AbstractVector{R},
	Δ::R,
	χ::Conjugate{IndBallL1{R}}
 ) where {R <: Real} = ShiftedGroupNormL2Binf(GroupNormL2([h.lambda]), xk, zero(xk), Δ, χ,  false)
 shifted(
		...
		lmax = norm(ψ.sol[idx]) + σ*(zlmax + abs((ϵ - 1)/ϵ+1)*λ*norm(ψ.xk[idx]))
 	 	...
 	 	if fl*fm > 0
			...
			if abs(n - σλ) ≈ 0
				...
				y[idx] .= l2prox(ψ.sol[idx] .- σ.*softthres((ψ.sol[idx]./σ .- step .* ψ.xk[idx]), ψ.Δ*step), σλ)
			end
 	 	end
