...
shifted(h::RootNormLhalf{R}, xk::AbstractVector{R}, Δ::R, χ::Conjugate{IndBallL1{R}}) where {R <: Real} =
ShiftedRootNormLhalfBinf(h, xk, zero(xk), Δ, χ, false)
shifted(
...
function prox!(
  ...
  qbar = q + (ψ.xk .+ ψ.sj)
  RNorm(tt, l) = .5/σ*(tt - q[l])^2 + ψ.λ*sqrt(abs(tt + ψ.sj[l] + ψ.xk[l]))
  ...
  for i ∈ eachindex(q)
    ...
    t[2] = -ψ.sj[i] + ψ.Δ
    t[3] = 0 - ψ.xk[i] - ψ.sj[i]
    t[4] = real(2 * sign(qbar[i]) / 3 * aqi * (1 + cos(2 * π / 3 - 2 * ϕ(qbar[i]) / 3))) - ψ.xk[i] - ψ.sj[i]
