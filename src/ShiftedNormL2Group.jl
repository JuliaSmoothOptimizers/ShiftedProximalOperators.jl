...
(ψ::ShiftedNormL2Group)(y) = ψ.h(ψ.xk + ψ.sj + y) + IndBallLinf(ψ.Δ)(ψ.sj + y)
...
shifted(
  ...
  Δ::R,
  χ::Conjugate{IndBallL1{R}},
) where {R <: Real} = ShiftedNormL2Group(h, xk, zero(xk), Δ, χ, false)
shifted(
  ψ::ShiftedNormL2Group{R, V0, V1, V2},
  sj::AbstractVector{R},
) where {R <: Real, V0 <: AbstractVector{R}, V1 <: AbstractVector{R}, V2 <: AbstractVector{R}} =
ShiftedNormL2Group(ψ.h, ψ.xk, sj, ψ.Δ, ψ.χ, true)

fun_name(ψ::ShiftedNormL2Group) = "shifted ∑ᵢ√|⋅| norm with L∞-norm trust region indicator"
fun_expr(ψ::ShiftedNormL2Group) = "t ↦ ‖xk + sj + t‖ₚᵖ + χ({‖sj + t‖∞ ≤ Δ}), p = 1/2"
