...
for (op, shifted_op) ∈ zip(
    ...
    (:ShiftedNormL0, :ShiftedNormL1, :ShiftedRootNormLhalf),
)
    @testset "$shifted_op" begin
        ...
        ψ = shifted(h, x)
        @test typeof(ψ) ==
              ShiftedOp{Float64,Vector{Float64},Vector{Float64},Vector{Float64}}
        @test all(ψ.sj .== 0)
        @test all(ψ.xk .== x)
...
for (op, shifted_op) ∈ zip((:IndBallL0,), (:ShiftedIndBallL0,))
    @testset "$shifted_op" begin
        ...
        ψ = shifted(h, x)
        @test typeof(ψ) ==
              ShiftedOp{Int64,Float64,Vector{Float64},Vector{Float64},Vector{Float64}}
        @test all(ψ.xk .== x)
...
for (op, shifted_op) ∈ zip((:Rank,), (:ShiftedRank,))
    @testset "$shifted_op" begin
        ...
        @test all(
            Q.U * Diagonal(t) * Q.Vt - reshape(x + s, m, n) .≈
            reshape(prox!(y, f, q, γ), m, n),
        )
    end
end
